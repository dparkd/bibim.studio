...
const setDeviation = ({ blur, gaussian, easing }, curve) => {
  const values = blurs.axes.map(axis =>
    getDeviation(blur[axis], easing, curve)
  );
...
const easings = {
  ...
  "in-elastic": (progress, amplitude, period) => {
    ...
    return -(
      strength *
      2 ** (10 * (progress -= 1)) *
      Math.sin(((progress - offset) * pi2) / period)
    );
  },
  ...
  "in-out-cubic": progress =>
    (progress *= 2) < 1
      ? 0.5 * progress ** 3
      : 0.5 * ((progress -= 2) * progress ** 2 + 2),
  "in-out-quartic": progress =>
    (progress *= 2) < 1
      ? 0.5 * progress ** 4
      : -0.5 * ((progress -= 2) * progress ** 3 - 2),
  "in-out-quintic": progress =>
    (progress *= 2) < 1
      ? 0.5 * progress ** 5
      : 0.5 * ((progress -= 2) * progress ** 4 + 2),
  "in-out-exponential": progress =>
    (progress *= 2) < 1
      ? 0.5 * 1024 ** (progress - 1)
      : 0.5 * (-(2 ** (-10 * (progress - 1))) + 2),
  "in-out-circular": progress =>
    (progress *= 2) < 1
      ? -0.5 * (Math.sqrt(1 - progress ** 2) - 1)
      : 0.5 * (Math.sqrt(1 - (progress -= 2) * progress) + 1),
...
const decomposeEasing = string => {
  const [easing, amplitude = 1, period = 0.4] = string.trim().split(" ");
